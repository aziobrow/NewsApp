...
public final class QueryUtils {
    ...
    private static List<Article> extractFeatureFromJson(String articleJSON) {
        ...
        try {
            ...
            // key called "response", which represents a list of all responses
            // for that query.
            JSONObject responseJsonResponse = baseJsonResponse.getJSONObject("response");

            // Extract the JSONArray associated with the key called "results",
            // which represents a list of articles
            JSONArray articleArray = responseJsonResponse.getJSONArray("results");

            // For each earthquake in the earthquakeArray, create an Article object
            for (int i = 0; i < articleArray.length(); i++) {
                ...
                Article article = new Article(sectionName, webTitle, publicationDate, url);

                // Add the new Article to the list of earthquakes.
